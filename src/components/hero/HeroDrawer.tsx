    ...
    <>
      ...
      <aside className="hero-drawer" role="dialog" aria-modal="true" aria-labelledby="hero-detail-title">
        ...
        <div className="hero-identity">
          ...
          <div><TierBadge tier={hero.metaTier} /><h2 id="hero-detail-title">{name}</h2><p>{hero.role ? (ko.roles[hero.role as keyof typeof ko.roles] ?? hero.role) : "—"}</p></div>
        </div>
        <dl className="drawer-stats">
          <div><dt>{ko.labels.pickRate}</dt><dd>{hero.source === "rivalsmeta" ? "—" : <StatCell value={hero.pickRate} />}</dd></div>
